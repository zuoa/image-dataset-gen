...
class VideoImportSchema(Schema):
    frame_interval_mode = fields.String(load_default="seconds", validate=validate.OneOf(["frames", "seconds"]))
    frame_interval = fields.Integer(load_default=30, validate=validate.Range(min=1, max=10000))
    frame_interval_seconds = fields.Float(load_default=5.0, validate=validate.Range(min=0.01, max=3600))
